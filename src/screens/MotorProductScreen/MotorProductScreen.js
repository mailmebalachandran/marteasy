...
import MotorProductList from '../../screens/MotorProductScreen/MotorProductList';
...
class MotorProductScreen extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      infoMessage: '',
      storeDetail: {
        id: this.props.route.params.storeId,
      },
      countDetail: [],
  ...
  componentDidMount = async () => {
    ...
    this.onMotorProductIdOnLoad(this.state.productList[0].id);
    
  };

  onMotorProductIdOnLoad = async (id) => {
    const result = await MotorAPI.getMotorProductId(id);
    console.log("In response motoAPI", result[0].attributes);
  ...
  onPageLoad = async () => {
    ...
    const storedId = 0;
    let productList = await this.props.route.params.productDetail;
    if (
  ...
  componentDidUpdate = async prevProps => {
    if (this.props.route.params.storeId !== prevProps.route.params.storeId) {
      ...
      const storedId = 0;
      await this.onPageLoad();
      this.onMotorProductIdOnLoad(this.state.productList.id);
    }
  };
  ...
  renderPickerItems = (productVariance) => {
    ...
    productVariance.map(p => {
      varianceItems.push(
        <Picker.Item key={p.id} label={p.attributes[0].option} value={p.regular_price} />
      )
    })
                ...
                <>
                  <View style={{ flex: 1 }}>
                    ...
                    <Header
                      ...
                      navigateValue="MotorMainCategoryScreen"
                    />
                    ...
                    <View style={{ flex: 1, backgroundColor: "white", }}>
                      <Picker
                        ...
                        mode={"dropdown"}
                        style={{ height: 50, width: "90%", borderRadius: 25, marginLeft: "5%", marginRight: "5%" }}
