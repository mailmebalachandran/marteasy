...
class OtpScreen extends Component {
  ...
  handleOtpChange = (text, index) => {
    ...
    if (text.nativeEvent.text !== '') {
      otp[index] = text.nativeEvent.text;
    } else {
      otp[index] = '';
    }
    this.setState({otp});
    let focusValue = [...this.state.focusValue];
    if (text.nativeEvent.text === '' && index > 0) {
      this[index - 1].focus();
    } else if (index <= 2 && text.nativeEvent.text !== '') {
      this[index + 1].focus();
    }
  ...
  renderOtpInputs = () => {
    return this.state.otp.map((item, index) => (
      <TextInput
        ...
        controlled={true}
        onChange={text => this.handleOtpChange(text, index)}
      />
    ));
  };
  resendOTP = async () => {
  ...
  render() {
    return (
      <View style={styles.containerStyle}>
        {this.state.IsLoaded && <ActivityOverlay />}
        ...
        <ScrollView>
          ...
          <View style={styles.otpContainer}>{this.renderOtpInputs()}</View>
          <View style={{flex: 1}}>
            <ButtonComponent
              titleValue={'Submit'}
              onPressHandler={() => {
          ...
          {!this.state.isOtpValid && (
            <Text style={styles.errorMsg}>{this.state.commonErr}</Text>
          )}
          <View
            style={{
              flex: 0.01,
